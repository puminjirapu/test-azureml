...
def init():
    ...
    g_logger.info(f"Model directory: {model_dir}")
    g_logger.info(f"Model root directory: {model_rootdir}")
    g_logger.info(f"Full model path: {model_path}")

    try:
        g_model = load_model(model_path)
    except Exception as e:
        g_logger.error(f"Model loading failed: {e}")
...
# Logging functions
def log_ml_model_details(ml_model):
    """Log MLmodel file details."""
    log_metadata(ml_model)
    log_task_type(ml_model)  

def log_task_type(ml_model):
    """Log the task type if provided in any of the flavors."""
    task_type = "task_type"
    task = "task"
    for flavor, config in ml_model['flavors'].items():
        if task_type in config:
            g_logger.info(f'Task type: {flavor}.{task_type}.{config[task_type]}')
        if task in config:
            g_logger.info(f'Task type: {flavor}.{task}.{config[task]}')

def log_metadata(ml_model):
    """Log the metadata details if present."""
    if 'metadata' in ml_model:
        g_logger.info(f'Metadata:\n{ml_model["metadata"]}')
...
def _get_data_loader(file_extension):
    ...
    if loader is None:
        raise ValueError("File type '" + str(file_extension) +"' is not supported. " \
...
def load_csv(data_file):
    if g_schema and g_schema.is_tensor_spec():
        ...
        df = pd.read_csv(data_file, engine='c', dtype=g_dtypes)
        reset_dataframe_index(df)
        return df

def reset_dataframe_index(df):
    '''
    Checks if the unnamed index value is in the first column and not in the signature.
    If so, resets in place the index of the dataframe to the unnamed column.

    Args:
        df (pd.DataFrame): Data

    Returns:
        None; If update occurs, it happens in the df object.
    '''
    unnamed_index = "Unnamed: 0"
    if unnamed_index == df.columns[0] and (g_schema is None or unnamed_index not in g_schema.input_names()):
        g_logger.info(f"Found {unnamed_index} in first column. Setting this column as index.")
        df.set_index(unnamed_index, inplace=True)
...
# Data output transformation functions
        ...
        if isinstance(arguments, pd.DataFrame):
            ...
            for data_file in arguments:
                try:
                    ...
                    # Logic enabled for all output types in April 2023.
                    ...
                    file_error_messages[baseFilename] = f"See logs/user/stdout/0/process000.stdout.txt for traceback on error: {str(e)}"
            if len(result) < len(arguments):
                # Error Threshold should be used here.
                printed_errs = "\n".join([str(filename) + ":\n" + str(error) + "\n" for filename, error in file_error_messages.items()])
                raise Exception(f"Not all input files were processed successfully. Record of errors by filename:\n{printed_errs}")
